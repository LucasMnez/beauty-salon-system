...
function renderizarServicos() {
  ...
  if (servicos.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="loading">Nenhum serviço cadastrado</td></tr>';
    return;
  }
...
async function deletarServico(id) {
  if (
    !confirm(
      "⚠️ Tem certeza que deseja deletar este serviço?\n\nEsta ação não pode ser desfeita!",
    )
  ) {
    return;
  }
...
function renderizarHorarios() {
  ...
  // Separar por tipo
  const diasSemanaConfigs = configHorarios.filter(
    (c) => c.tipo === "dia_semana",
  );
  const datasEspecificas = configHorarios.filter(
    (c) => c.tipo === "data_especifica",
  );
  ...
  if (datasEspecificas.length > 0) {
    html +=
      '<h3 style="margin-top: 30px; margin-bottom: 15px;">Datas Específicas</h3>';
    html += '<div class="table-container">';
    html +=
      "<table><thead><tr><th>Data</th><th>Horário</th><th>Almoço</th><th>Status</th><th>Ações</th></tr></thead><tbody>";
...
async function atualizarConfigHorario(configId, campos) {
  ...
  try {
    ...
    const response = await fetch(
      `${API_URL}/admin/config-horarios/${configId}`,
      {
        method: "PATCH",
        credentials: "include",
        headers: headers,
...
function abrirModalHorarioEspecifico(configId = null) {
  ...
  if (configId) {
    ...
    if (config && config.tipo === "data_especifica") {
      ...
      document.getElementById("almoco-inicio").value =
      ...
      document.getElementById("almoco-config").style.display = config.tem_almoco
        ? "block"
        : "none";
    }
  } else {
...
document
  .getElementById("horario-tem-almoco")
  .addEventListener("change", function () {
    document.getElementById("almoco-config").style.display = this.checked
      ? "block"
      : "none";
  });
...
async function deletarHorarioEspecifico(id) {
  if (
    !confirm(
      "⚠️ Tem certeza que deseja deletar esta configuração?\n\nEsta ação não pode ser desfeita!",
    )
  ) {
    return;
  }
