...
async function carregarServicos() {
    try {
        ...
        mostrarMensagem('Erro ao carregar serviços. Verifique se o servidor está rodando.', 'error');
    }
}
...
function mostrarSelecaoServico(data) {
    ...
    const valorTotal = servicosSelecionados.reduce((sum, servico) => sum + servicos[servico], 0);
    ...
    const atualizarModal = () => {
        const valorTotal = servicosSelecionados.reduce((sum, servico) => sum + servicos[servico], 0);
        modal.querySelector('div[style*="background: #f5f5f5"] p').innerHTML = `<strong>Valor Total: R$ ${valorTotal.toFixed(2).replace('.', ',')}</strong>`;
    ...
    modal.querySelector('#confirmarServicos').addEventListener('click', async () => {
        ...
        console.log('⏰ Horários disponíveis para esta data:', horariosDisponiveis);
        const periodosDisponiveis = Object.keys(horariosDisponiveis);
        console.log('📋 Períodos disponíveis:', periodosDisponiveis);
        
        if (periodosDisponiveis.length === 0) {
            console.error('❌ Nenhum período disponível encontrado para a data:', data);
...
async function carregarDisponibilidadeMes() {
    ...
    try {
        console.log(`📅 Carregando disponibilidade do mês ${mesFormatado}/${anoFormatado}...`);
        const inicio = performance.now();
        
        // Uma única requisição para todo o mês
        const response = await fetch(`${API_URL}/disponibilidade-mes?mes=${mesFormatado}&ano=${anoFormatado}`);
        ...
        mostrarMensagem('Erro ao carregar disponibilidade. Tente novamente.', 'error');
...
function renderizarCalendario() {
    ...
    for (let dia = 1; dia <= ultimoDia.getDate(); dia++) {
        ...
        const periodosDisponiveis = horariosDisponiveis ? Object.keys(horariosDisponiveis) : [];
        // Se não há dados carregados ainda, assumir disponível (será atualizado quando carregar)
        const temDisponibilidade = horariosDisponiveis === undefined || 
        ...
        if (isDomingo) {
            ...
            if (horariosDisponiveis !== undefined && periodosDisponiveis.length === 0) {
        ...
        if (horariosDisponiveis && periodosDisponiveis.length > 0) {
            console.log(`📅 Data ${dataStr}: períodos disponíveis =`, periodosDisponiveis);
            indicadores = periodosDisponiveis.map(periodo => {
...
async function mostrarSelecaoHorario() {
    ...
    try {
        const servicosParam = servicosSelecionados.length > 0 
            ? servicosSelecionados.join(',') 
            : '';
        
        const response = await fetch(`${API_URL}/horarios-disponiveis?data=${dataSelecionada}&servicos=${servicosParam}`);
        ...
        if (horariosDisponiveis.length === 0) {
            mostrarMensagem('Nenhum horário disponível para esta data com os serviços selecionados', 'error');
            return;
        }
        ...
        mostrarMensagem('Erro ao carregar horários disponíveis. Tente novamente.', 'error');
    }
}
...
function mostrarFormulario() {
    ...
    document.getElementById('resumo-servico').textContent = servicosSelecionados.join(', ');
...
form.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const selecaoServicoInicial = document.getElementById('selecaoServicoInicial');
            ...
            const valorTotal = result.valor || formData.servicos.reduce((sum, servico) => sum + (servicos[servico] || 0), 0);
...
function notificarRaissaWhatsApp(dadosAgendamento, valorTotal) {
    ...
    const servicosLista = servicosSelecionados.length > 0 ? servicosSelecionados : (dadosAgendamento.servicos || []);
    const servicosFormatados = Array.isArray(servicosLista) ? servicosLista.join(', ') : servicosLista;
    
    // Criar mensagem formatada
    const mensagem = `🔔 *NOVO AGENDAMENTO CRIADO*\n\n` +
...
function mostrarLinkAgendamento(dados) {
    const baseUrl = window.location.origin + window.location.pathname.replace('agendamento.html', '');
    ...
    const mensagemWhatsApp = `Olá ${dados.nome}! Seu agendamento foi confirmado:\n\n` +
...
async function carregarProximosHorarios() {
    ...
    try {
        console.log('📅 Carregando horários para:', { hoje: hojeStr, amanha: amanhaStr });
        ...
        if (horariosHojeLimitados.length > 0) {
            const hojeFormatado = hoje.toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' });
        ...
        if (horariosAmanhaLimitados.length > 0) {
            const amanhaFormatado = amanha.toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' });
            html += `<div class="horario-dia"><strong>Amanhã (${amanhaFormatado}):</strong> ${horariosAmanhaLimitados.join(', ')}</div>`;
        }
        
        if (html === '') {
            html = '<div class="horario-dia">Nenhum horário disponível para hoje ou amanhã</div>';
        }
        ...
        horariosHojeAmanhaEl.innerHTML = '<div class="horario-dia">Erro ao carregar horários. Tente novamente.</div>';
    }
}
    ...
    try {
        ...
        for (let i = 0; i < 14; i++) {
            ...
            if (disponibilidade[dataStr]) {
                ...
                console.log(`⚠️ Data ${dataStr} não encontrada no mês, buscando individualmente...`);
                    ...
                    fetch(`${API_URL}/horarios-disponiveis?data=${dataStr}`)
                        ...
                        .then(data => {
                            if (data.horarios && Array.isArray(data.horarios) && data.horarios.length > 0) {
                                ...
                                const PERIODOS = {
                                    'manhã': ['07:00', '07:30', '08:00', '08:30', '09:00', '09:30', '10:00', '10:30', '11:00', '11:30'],
                                ...
                                for (const periodo of ordem_periodos) {
                                    ...
                                    const horarios_disponiveis_periodo = horarios_periodo.filter(h => data.horarios.includes(h));
        ...
        console.log('📊 Disponibilidade final após filtro:', Object.keys(disponibilidade));
...
function renderizarCalendario14Dias() {
    ...
    for (let i = 0; i < 14; i++) {
        ...
        const totalHorarios = horariosDisponiveis ? 
            Object.values(horariosDisponiveis).reduce((sum, arr) => sum + (Array.isArray(arr) ? arr.length : 0), 0) : 0;
...
function atualizarIndicadoresLotacao() {
    ...
    Object.values(disponibilidade).forEach(horarios => {
        const count = Object.values(horarios).reduce((sum, arr) => sum + arr.length, 0);
        if (count > 0) {
            totalHorarios += count;
            diasComHorarios++;
        }
...
async function inicializar() {
    ...
    document.getElementById('verMaisHorariosBtn').addEventListener('click', () => {
