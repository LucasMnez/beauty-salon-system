...
async function carregarAgendamentos() {
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Erro de conexão: ${fetchError.message}. Verifique se o servidor Flask está rodando na porta correta.`);
        }
        ...
        if (data.length > 0) {
            console.log('⚠️ ATENÇÃO: Recebidos agendamentos do servidor:', data.length);
            console.log('   Primeiros IDs:', data.slice(0, 5).map(a => a.id));
        } else {
...
function renderizarAgendamentos() {
    ...
    tbody.innerHTML = agendamentos.map(ag => {
        ...
        const valorFormatado = `R$ ${ag.valor.toFixed(2).replace('.', ',')}`;
        
        // Determinar ações disponíveis
        const acoes = [];
        if (ag.status === 'pendente') {
            acoes.push({ label: '✓ Confirmar', status: 'confirmado', class: 'confirmar' });
        }
        if (ag.status !== 'cancelado' && ag.status !== 'concluido') {
            acoes.push({ label: '✕ Cancelar', status: 'cancelado', class: 'cancelar' });
        }
        if (ag.status === 'confirmado') {
            acoes.push({ label: '✓ Concluir', status: 'concluido', class: 'concluir' });
        }
        ...
        const formaPagamentoFormatada = formasPagamento[ag.forma_pagamento] || ag.forma_pagamento || 'Pendente';
...
function aplicarFiltros() {
    ...
    filtros.telefone = document.getElementById('filter-telefone').value.trim().replace(/\D/g, '');
...
async function limparTodosAgendamentos() {
    const confirmacao = prompt('⚠️ ATENÇÃO: Isso vai deletar TODOS os agendamentos!\n\nDigite "LIMPAR" para confirmar:');
    ...
    try {
        ...
        console.log('Enviando requisição para:', `${API_URL}/admin/limpar-agendamentos`);
...
async function editarAgendamento(id) {
    ...
    try {
        ...
        document.getElementById('edit-valor').value = agendamento.valor.toFixed(2);
        document.getElementById('edit-status').value = agendamento.status;
        document.getElementById('edit-forma-pagamento').value = agendamento.forma_pagamento || 'pendente';
        document.getElementById('edit-pago').value = agendamento.pago ? 'true' : 'false';
        document.getElementById('edit-data-pagamento').value = agendamento.data_pagamento || '';
        ...
        servicosDisponiveis.forEach(servico => {
            ...
            option.textContent = `${servico.nome} - R$ ${servico.valor.toFixed(2).replace('.', ',')}`;
            // Marcar como selecionado se estiver na lista de serviços do agendamento
            const servicosAgendamento = agendamento.servico.split(', ').map(s => s.trim());
...
async function abrirModalCriar() {
    ...
    horarioSelect.innerHTML = '<option value="">Selecione primeiro os serviços e a data</option>';
...
function configurarAutocompleteClientes() {
    ...
    if (autocompleteConfigurado && nomeInputCreate.dataset.autocompleteConfigurado === 'true') {
...
async function buscarHorariosDisponiveisCriar() {
    ...
    const servicosSelecionados = Array.from(servicosSelect.selectedOptions).map(opt => opt.value);
    ...
    try {
        ...
        const response = await fetch(`${API_URL}/horarios-disponiveis?data=${dataSelecionada}&servicos=${encodeURIComponent(servicosParam)}`);
...
async function buscarClientes(nome) {
    ...
    try {
        ...
        if (clientes.length > 0) {
            ...
            if (clientes.length === 1 && nome.length > 0 && clientes[0].nome.toLowerCase() === nome.toLowerCase()) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editForm) {
        editForm.addEventListener('submit', async function(e) {
            ...
            const dataPagamento = document.getElementById('edit-data-pagamento').value || null;
            ...
            try {
                ...
                if (response.ok && responseDataEdit.success) {
                    ...
                    alert(`❌ Erro: ${responseDataEdit.error || 'Erro ao atualizar agendamento'}`);
                }
            } catch (error) {
        ...
        if (servicosSelectCreate && dataInputCreate) {
            ...
            servicosSelectCreate.addEventListener('change', buscarHorariosDisponiveisCriar);
            dataInputCreate.addEventListener('change', buscarHorariosDisponiveisCriar);
        }
        ...
        if (createForm) {
            createForm.addEventListener('submit', async function(e) {
                ...
                const telefone = document.getElementById('create-telefone').value.replace(/\D/g, '');
                ...
                const formaPagamento = document.getElementById('create-forma-pagamento').value;
                const pago = document.getElementById('create-pago').value === 'true';
                const dataPagamento = document.getElementById('create-data-pagamento').value || null;
