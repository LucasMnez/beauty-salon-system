...
async function carregarDadosFinanceiros() {
  ...
  try {
    ...
    const response = await fetch(
      `${API_URL}/admin/financeiro?${params.toString()}`,
      {
        method: "GET",
        credentials: "include",
        headers: headers,
...
function atualizarResumo() {
  ...
  document.getElementById("total-faturado").textContent = formatarMoeda(
    resumo.total_faturado || 0,
  );

  // Total recebido
  document.getElementById("total-recebido").textContent = formatarMoeda(
    resumo.total_recebido || 0,
  );

  // Total pendente
  document.getElementById("total-pendente").textContent = formatarMoeda(
...
function atualizarTabelasResumo() {
  ...
  const tbodyStatusPagamento = document.getElementById(
  ...
  if (statusPagamentoData.every((item) => item.valor === 0)) {
    tbodyStatusPagamento.innerHTML =
      '<tr><td colspan="2" class="empty-state">Nenhum dado</td></tr>';
  } else {
