...
app.config['SESSION_COOKIE_SAMESITE'] = 'Lax'  # Permite cookies em navegação normal
...
# Configuração WhatsApp para notificações automáticas (Baileys)
WHATSAPP_RAISSA = '5511993940514'  # Telefone da Raissa (formato internacional sem +)
WHATSAPP_BOT_URL = os.getenv('WHATSAPP_BOT_URL', 'http://localhost:3001')  # URL do serviço Baileys
ENVIAR_WHATSAPP_AUTO = True  # Ativar/desativar envio automático
...
def gerar_qrcode_pix(valor, descricao, identificador):
    """Gera QR Code PIX válido seguindo padrão EMV do Banco Central"""
    try:
        # Formato EMV conforme especificação do Banco Central
        # Payload Format Indicator (obrigatório)
        payload = "000201"
        
        # Merchant Account Information (obrigatório)
        # ID 26: Merchant Account Information
        # Formato: GUI (0014BR.GOV.BCB.PIX) + ID da chave (01) + tamanho + chave
        gui = "0014BR.GOV.BCB.PIX"
        chave_id = "01"
        chave_tamanho = formatar_tamanho(PIX_CHAVE, len(PIX_CHAVE))
        merchant_account = gui + chave_id + chave_tamanho
        payload += "26" + formatar_tamanho(merchant_account, len(merchant_account))
        ...
        # ID 53: Transaction Currency - 986 = BRL (Real Brasileiro)
        payload += "5303986"
        
        # Transaction Amount (obrigatório)
        # ID 54: Transaction Amount
        valor_str = f"{valor:.2f}"
        payload += "54" + formatar_tamanho(valor_str, len(valor_str))
        ...
        # ID 59: Merchant Name
        # Limitar a 25 caracteres conforme especificação e converter para maiúsculo
        nome_recebedor = PIX_NOME_RECEBEDOR[:25].strip().upper()
        payload += "59" + formatar_tamanho(nome_recebedor, len(nome_recebedor))
        
        # Merchant City (obrigatório)
        # ID 60: Merchant City
        # Limitar a 15 caracteres, remover espaços extras e converter para maiúsculo
        cidade = PIX_CIDADE[:15].strip().upper().replace(' ', '')
        payload += "60" + formatar_tamanho(cidade, len(cidade))
        
        # Additional Data Field Template (opcional)
        # ID 62: Additional Data Field Template
        # Formato: ID do campo (05) + tamanho do identificador + identificador
        # O campo 05 é o Reference Label (ID de referência)
        # Limitar identificador a 25 caracteres
        identificador_limpo = identificador[:25]
        add_data_field = "05" + formatar_tamanho(identificador_limpo, len(identificador_limpo))
        payload += "62" + formatar_tamanho(add_data_field, len(add_data_field))
        
        # CRC16 (obrigatório)
        # ID 63: CRC16
        payload_crc = payload + "6304"
        crc = calcular_crc16(payload_crc)
        pix_copia_cola = payload_crc + crc
        
        # Validar tamanho máximo (512 caracteres)
        if len(pix_copia_cola) > 512:
            raise ValueError(f"Payload PIX muito grande: {len(pix_copia_cola)} caracteres")
        ...
        # Debug: imprimir payload completo para verificação
        print(f"\n{'='*60}")
        print(f"DEBUG PIX - Payload Completo:")
        print(f"{'='*60}")
        print(f"Payload: {pix_copia_cola}")
        print(f"Tamanho: {len(pix_copia_cola)} caracteres")
        print(f"CRC calculado: {crc}")
        print(f"Valor: R$ {valor:.2f}")
        print(f"Chave PIX: {PIX_CHAVE}")
        print(f"Nome: {nome_recebedor}")
        print(f"Cidade: {cidade}")
        print(f"{'='*60}\n")
        
        return pix_copia_cola, img_str
    except Exception as e:
        print(f"Erro ao gerar QR Code PIX: {e}")
        import traceback
        traceback.print_exc()
        # Retornar código simples com chave PIX para pagamento manual
        pix_simples = f"Chave PIX: {PIX_CHAVE}\nValor: R$ {valor:.2f}\nDescrição: {descricao}"
        qr = qrcode.QRCode(version=1, box_size=10, border=4)
        qr.add_data(pix_simples)
        qr.make(fit=True)
        img = qr.make_image(fill_color="black", back_color="white")
        buffered = BytesIO()
        img.save(buffered, format="PNG")
        img_str = base64.b64encode(buffered.getvalue()).decode()
        return pix_simples, img_str
...
def enviar_notificacao_whatsapp(nome, telefone, servicos, data_agendamento, horario, valor_total):
    """Envia notificação automática via WhatsApp usando Baileys"""
    if not ENVIAR_WHATSAPP_AUTO:
        return
    ...
    try:
        ...
        dia_semana = ['Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 
                      'Sexta-feira', 'Sábado', 'Domingo'][data_obj.weekday()]
        ...
        mensagem = f"""🔔 *NOVO AGENDAMENTO CRIADO*
...
"""
        ...
        # Enviar mensagem via API do Baileys
        print(f"\n📱 Enviando notificação WhatsApp para {WHATSAPP_RAISSA}...")
        print(f"🔗 URL do bot: {WHATSAPP_BOT_URL}/send-message")
        
        try:
            response = requests.post(
                f"{WHATSAPP_BOT_URL}/send-message",
                json={
                    'phoneNumber': WHATSAPP_RAISSA,
                    'message': mensagem
                },
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                print(f"   ID da mensagem: {result.get('messageId', 'N/A')}")
            elif response.status_code == 503:
                print(f"⚠️  WhatsApp Bot não está conectado!")
                print(f"   Verifique se o bot está rodando: pm2 logs whatsapp-bot")
            else:
                print(f"⚠️  Erro ao enviar WhatsApp: {response.status_code}")
                print(f"   Resposta: {response.text}")
        except requests.exceptions.Timeout:
            print(f"❌ Timeout ao conectar com o bot WhatsApp")
        except Exception as e:
            print(f"❌ Erro inesperado: {e}")
        
    except requests.exceptions.ConnectionError:
        print(f"❌ Erro: Não foi possível conectar ao serviço WhatsApp Bot em {WHATSAPP_BOT_URL}")
        print("💡 Certifique-se de que o serviço Node.js está rodando:")
        print("   cd . && npm start")
    except Exception as e:
